...
def cli(ctx,
    ...
    mots.extend([io.load_posemot_sleap_analysis(filename) for filename in load_sleap_analysis])
    dfs = []
    ...
    if toxtrac_topleft_xy and len(toxtrac_topleft_xy) != len(load_toxtrac):
        raise click.BadParameter('--toxtrac-topleft-xy has to be specified for all --load-toxtrac input files or omitted entirely')
...
@cli.command()
@click.pass_context
@click.argument("video-in",
                type=click.Path(exists=True, readable=True, dir_okay=False),
                )
@click.argument('video-out',
...
def visualize(ctx, video_in, video_out, source_display_name, limit_duration):
    ...
    if names and len(mots) != len(names):
        raise click.BadParameter('source-display-name must be the same length as number of input files')
    if not mots:
